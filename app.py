...
def extract_features_authentic(file_path_str):
    """Your authentic extraction stack made completely stable with a single array output."""
...
def predict():
    ...
    try:
        ...
        if data is None:
            return jsonify({"error": "Feature array extraction failed due to empty audio file"}), 400
            
        # Generate predictions from the loaded weights
        ...
        return jsonify({"error": f"Internal pipeline failure: {str(e)}"}), 500
    finally:
